...
resource "proxmox_vm_qemu" "server" {
  description = "Deploiement VM Debian on Proxmox"
  name        = var.name
...
resource "local_file" "inventory" {
  ...
  filename        = "./ansible/inventory-user.yaml"
...
resource "local_file" "playbook" {
  ...
  filename        = "./ansible/playbook-user.yaml"
  file_permission = "0644"
}

resource "null_resource" "play_ansible" {
  provisioner "local-exec" {
    command = "ansible-playbook -i ansible/inventory-user.yaml ansible/playbook-user.yaml"
  }
  depends_on = [
